...
pacstrap /mnt linux-lts linux-firmware base base-devel lvm2 grub efibootmgr amd-ucode
...
packagelist=(
  ...
  # Window manager 
  xorg-xwayland sway swaylock swayidle wofi foot thunar lf wl-clipboard lxappearance neofetch
  # Thunar
  ...
  # Network
  iwd wget curl tor wireguard-tools reflector resolvconf
  # Virtualization
)
...
chmod 700 /boot
head -c 64 /dev/urandom > /boot/volume.key && chmod 600 /boot/volume.key
...
GRUB_CMDLINE_LINUX="zswap.enabled=1 cryptdevice=UUID=$LUKS_UUID:crypt cryptkey=rootfs:/boot/volume.key"
EOF
